...
describe('CRUD pedido', () => {
  ...
  it('Deve falhar ao buscar um pedido com um código que não existe', async () => {
    await supertest(app)
      .get(`${endpointPedido}/999999`)
      .expect(404)
  })
  // esse nem bate na rota 
  it('Deve falhar ao buscar um pedido caso o não seja informado um código', async () => {
    await supertest(app)
      .get(`${endpointPedido}/`)
      .expect(404)
  })

  it('Deve falhar cadastrar um pedido', async () => {
    await supertest(app)
      .post(endpointPedido)
      .send(pedidoInvalido)
      .expect(400)
